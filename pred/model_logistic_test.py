...
def loss(y):
    return -np.sum(dc.t_train * np.log(y))
...
def grad(w, b, x, t):
    ...
    y = pred(w, b, x)
    grad = y - t
    return np.dot(x.T, grad) / N, np.mean(grad)


def grad_update(x, t, alpha=0.01, niter=200):
    w = np.zeros((x.shape[1], t.shape[1]))
    b = 0
    loss_lst = []

    for i in range(niter):
        grad_result, b = grad(w, b, x, t)
        w = w - (alpha * grad_result)

        y = pred(w, b, x)
        loss_y = loss(y)
        loss_lst.append(loss_y)

    return w, b


print(X_train.shape)
print(dc.t_train.shape)
print(grad_update(X_train, dc.t_train)[0].shape)
w, b = grad_update(X_train, dc.t_train)
print(b)
print("w: ", w.shape)
np.savetxt("../data/pred_data/final/coefficients.csv", w, delimiter=",")
